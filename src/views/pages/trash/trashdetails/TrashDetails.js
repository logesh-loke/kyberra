...
import {
  FiChevronLeft,
  FiRotateCcw, // 👈 Undo icon instead of Archive
  FiAlertCircle,
  FiTrash2,
  FiMoreVertical,
  FiChevronDown,
...
//  Reusable confirm modal (same style as previous)
...
const TrashDetails = () => {
  ...
  //  Confirm modal state (single delete only)
  ...
  useEffect(() => {
    if (actionData || actionError) {
      if (actionData?.data?.message === "Add To retrive Successfully ") {
        toast.success("Message retrieved successfully.");
        setTimeout(() => {
          navigate(-1);
        }, 500);
      } else if (
        actionData?.data?.message === "Add To permanently_deleted Successfully "
      ) {
        ...
        console.log("no action error for trash details page");
      }
    }
  ...
  }, [actionData, actionError, navigate]);
  ...
  const handleConfirmDelete = () => {
    handleAction("permanently_deleted");
        ...
        <div className="flex items-center justify-between px-2 py-2">
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-1">
              {/* 🔁 Undo instead of Archive */}
              <button
                className="p-2 rounded hover:bg-gray-100 transition"
                title="Undo"
                onClick={() => handleAction("retrive")}
              >
                <FiRotateCcw />
              </button>
              ...
              {/* 🧨 Permanent delete with confirm modal */}
              <button
                className="p-2 rounded hover:bg-gray-100 transition"
...
{decryptedFile?.length > 0 && (
  <div className="mt-6 border-t pt-4">
    ...
    <div className="flex gap-3 flex-wrap">
      ...
      {/*  Custom Confirm Modal for permanent delete */}
      <ConfirmModal
        open={confirmOpen}
        title="Delete permanently?"
